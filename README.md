...
Phase 7 will expand the Python backend from a single `Shachi` Mayor into a distributed multi-agent network. Instead of one LLM making all decisions, we will implement a concurrent multi-agent graph:

- **Level 1 (The Advisors)**: Independent LLMs running concurrently (`asyncio.gather`).
  - **Public Health Official**: Analyzes the grid to find maximum density hotspots and recommends targeted quarantine zones (low speeds) to prevent virus spread.
  - **Chief Economist**: Analyzes the grid to maintain high throughput and flow, recommending high-speed highways to maximize economic activity.
- **Level 2 (The Synthesizer)**:
  - **The Mayor**: Receives the raw 10x10 grid stats AND the written reports/proposals from both the Health Official and the Economist. The Mayor synthesizes these competing priorities into the final 10x10 `policy_speed_map`.
