...
Phase 7 will expand the Python backend from a single `Shachi` Mayor into a distributed multi-agent network. Instead of one LLM making all decisions, specialized institutional agents will analyze different sectors of the spatial grid simultaneously. They will synthesize their findings and vote on global policies, introducing complex social dynamics and competing priorities into the physical simulation.
