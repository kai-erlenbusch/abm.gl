...
## Phase 7: SIR Epidemic Model (In Progress)
Phase 7 transforms the particle system into a true Agent-Based Model by implementing a classic "Susceptible-Infected" (SI) epidemic spread.
- **Infection State Vector**: A discrete `Uint32Array` tracks the health of all 1,000,000 agents. Patient Zeros are randomly seeded at startup.
- **Thread-Local Transmission**: During the WebGPU $O(N^2)$ spatial collision pass, Healthy threads that detect proximity to an Infected neighbor mutate their own state to Infected. This strict thread-local mutation completely circumvents atomic race conditions.
- **Visual Feedback**: The React Three Fiber `pointsMaterial` utilizes TSL `select` nodes to instantaneously render Infected agents as Red and Healthy agents as Green.
- **Macro-Level Lockdowns**: The GPU aggregates spatial infection counts and streams them to the Python backend. The LLM Mayor detects outbreak hotspots and enforces localized quarantines by dropping the target speed to `0.0`.

---

## Phase 8: Multi-Agent Policy Network (Complete)
